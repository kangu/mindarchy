...
An `.omm` document is the existing UTF-8 JSON document, saved with a dedicated extension. There is no archive, binary header, embedded screenshot, or platform-specific wrapper. The schema remains `format: "mindmap-lab"`, `version: 1`. Text, Task, Date, calendar entries, themes, layout, manual offsets, notes, folding and relationships retain their existing representation.
...
mindmap-lab 'Project.omm'          # positional argument / file-manager opening
mindmap-lab --render-preview 'Project.omm' --preview-output preview.png
```
...
```sh
python3 scripts/install-linux-filetype.py --binary build/mindmap-lab
